...
output "my_security_group_id" {
  description = "id of my servers security group"
  value       = aws_security_group.aws_group.id
}

output "web-server-private-IP" {
  value = aws_instance.server_web.private_ip
}
...
output "instances_IDs" {
  value = [
    aws_instance.server_web.id,
    aws_instance.server_app.id,
    aws_instance.server_DB.id
  ]
}
